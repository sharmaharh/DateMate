...
- (void)send:(id)data;
{
    ...
    dispatch_async(_workQueue, ^{
        ...
        {
            [NSJSONSerialization dataWithJSONObject:data options:NSJSONWritingPrettyPrinted error:nil];
            NSString *str = [[NSString alloc] initWithData:[NSJSONSerialization dataWithJSONObject:data options:NSJSONWritingPrettyPrinted error:nil] encoding:NSUTF8StringEncoding];
            [self _sendFrameWithOpcode:SROpCodeTextFrame data:[str dataUsingEncoding:NSUTF8StringEncoding]];
        }
        else {
            assert(NO);
        }
...
- (void)_handleMessage:(id)message
{
    ...
    [self _performDelegateBlock:^{
        ...
        {
            dict = [NSJSONSerialization JSONObjectWithData:message options:NSJSONReadingAllowFragments error:nil];
        }
        else
        {
            dict = [self convertJSONStringToNSDictionary:message];
        }
        [self.delegate webSocket:self didReceiveMessage:dict];
    }];
}
